...
**Location:** `components/public/QuickViewModal.jsx`

Modal overlay that displays product details with image gallery and add-to-cart functionality.

**Props:**
- `product` (object) - Product data with optional images array
- `onClose` (function) - Callback to close modal

**Key Features:**
- Split-screen layout (image + details)
- Image gallery with navigation (if multiple images)
- Price display in MXN
- Category badge
...
```jsx components/public/QuickViewModal.jsx
"use client";
import Image from "next/image";
...
export default function QuickViewModal({ product, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#F5F1EB] w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl flex flex-col md:flex-row shadow-2xl">
        ...
        <div className="md:w-3/5 relative aspect-square">
          <Image src={images[currentImg]} alt={product.name} fill className="object-cover" />
          {images.length > 1 && (
        ...
        <div className="w-full md:w-2/5 p-8 flex flex-col justify-center">
          <span className="text-[9px] uppercase tracking-[0.4em] text-[#C4A95E] font-bold mb-4">
            {product.categories?.name}
          </span>
          <h2 className="text-3xl font-serif italic text-black mb-4">{product.name}</h2>
          <p className="text-xl text-[#A07F3A] mb-8">${product.base_price?.toLocaleString()} MXN</p>
          <p className="text-sm mb-10">{product.description || "Pieza exclusiva de oro laminado."}</p>
          <AddToCartButton product={product} onClose={onClose} />
          <button onClick={() => window.location.href = `/products/${product.id}`} className="mt-4 text-[9px] uppercase">
...
**Location:** `components/public/AddToCartButton.jsx`

Reusable button component that adds products to cart with success feedback.

**Props:**
- `product` (object) - Product to add to cart
- `onClose` (function, optional) - Callback for closing parent modal

**Key Features:**
- Success animation with check icon
- Auto-opens cart sidebar after adding
- 500ms delay for visual feedback
- Gold background on success state
- Disabled state during animation

**Implementation:**

```jsx components/public/AddToCartButton.jsx
"use client";
import { useState } from "react";
import { useCart } from "@/context/CartContext";
...
export default function AddToCartButton({ product, onClose }) {
  ...
  return (
    <button
      onClick={handleAdd}
      disabled={isAdded}
      className={`w-full py-5 text-[10px] uppercase tracking-[0.4em] font-bold transition-all flex items-center justify-center gap-3 
        ${isAdded ? "bg-[#C4A95E] text-white" : "bg-black text-white hover:bg-[#A07F3A]"}`}
    >
      {isAdded ? (
        <>
...
**Location:** `components/public/AnnouncementBar.jsx`

Dynamic announcement banner that displays configurable messages from the database.

**Key Features:**
- Loads message from `store_config` table
...
```jsx components/public/AnnouncementBar.jsx
...
export default async function AnnouncementBar() {
  ...
  return (
    <div className="w-full bg-black text-white py-2.5 px-6">
...
**Usage:**
Add to layout file to display site-wide announcements (e.g., promotions, shipping updates).
...
**Location:** `components/admin/NotificationSuccess.jsx`

Success notification component for admin actions.

**Implementation:**

```jsx components/admin/NotificationSuccess.jsx
...
**Usage:**
Display after successful product creation/update operations in admin panel.
